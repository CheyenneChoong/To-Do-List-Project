...
def addTask(): # Function to add a new task.
    ...
    taskID = taskDate.replace('-', '') + '1' # Create unique ID
    for task in taskList:
        if task['taskDate'] == taskDate:
            lastDigit = task['taskID'][8:]
            taskID = taskDate.replace('-', '') + str(int(lastDigit) + 1)
...
def updateTask(): # Function to update an existing task.
    ...
    if oldData['taskDate'] == taskDate: # Checks if there is a need to change ID.
        ...
        for task in taskList:
            if task['taskDate'] == taskDate:
                lastDigit.append(int(task['taskID'][8:]))

    lastDigit = max(lastDigit)
    newTaskID = taskDate.replace('-', '') + str(lastDigit + 1)
    
    with open('tasks.txt', 'w') as taskFile: # Writes the update into the file.
        for task in taskList:
            if task['taskID'] == taskID:
...
def removeTask(): # Function to remove an existing task.
    ...
    with open('tasks.txt', 'w') as taskFile: # Save the new changes.
        for task in taskList:
            if task['taskID'] != taskID:
...
def removeOldTask(): # Function to remove old task.
    ...
    todayDate = time.strptime(todayDate, '%d-%m-%Y')
    with open('tasks.txt', 'a') as taskFile:
        ...
        for task in taskList:
            ...
            taskDate = time.strptime(taskDate, '%d-%m-%Y')
            check = time.mktime(todayDate) - time.mktime(taskDate)
            check = check / (24 * 3600)
            if check <= 0:
                task = '; '.join(task.values()) + '\n'
                taskFile.write(task)
